...
def GetEqualityConstraints(G, sampleODs):
    ...
    A = sparse.lil_matrix((N, M), dtype=np.byte)
    ...
    A12 = sparse.lil_matrix(np.matlib.repmat(np.eye(M, dtype=np.byte), 1, K))
    A21 = sparse.lil_matrix((N*K, M), dtype=np.byte)
    ...
    bBig = sparse.lil_matrix((M+N*K, 0), dtype=np.double)

    # construct bBig columns
    for s in range(S):

        b = sparse.lil_matrix((N, K), dtype=np.double)
        ...
        bVecUp  = sparse.lil_matrix((M, 1), dtype=np.double)  # bVec = |---------|
        bVec    = sparse.vstack((bVecUp, bVecLow))            #        | bVecLow |

        bBig = sparse.hstack((bBig, bVec))  # bBig = | bVec1 | ... | bVecS |

    return sparse.csc_matrix(ABig), bBig.todense()
